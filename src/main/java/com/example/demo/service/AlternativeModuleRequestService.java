...
public class AlternativeModuleRequestService {
	...
	public void submitAlternativemoduleRequest(int sid, MultipartFile annex, String mcode[], String currentGrade[],
		...
		try {
			...
			AlternativeModuleRequest req = new AlternativeModuleRequest(filePath, "", enteredDate, false,
					studentService.getStudent(sid), comment, "Alternative");
			alterRepo.save(req);

			List<ModuleDiscontinue> moduleDiscontinueList = new ArrayList();

			for (int i = 0; i < mcode.length; i++) {
				ModuleDiscontinue moduleDiscontinue = new ModuleDiscontinue(req, moduleService.getModuleById(mcode[i]),
						"", "", currentGrade[i]);
	...
	public List<AlternativeModuleRequest> getAllRequest() {
		...
		List<AlternativeModuleRequest> allReq = alterRepo.findAll();

		List<AlternativeModuleRequest> decReq = new ArrayList<>();
	...
	public String updateRequest(int rid, String status, String decision, String alterMcode[], String offeringSem[])
